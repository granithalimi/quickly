...
function ProductsSection() {
  ...
  return (
    <div ref={refProd} className="py-60 mx-auto grid lg:grid-cols-5 md:grid-cols-2 grid-cols-1 w-2/3 gap-16 overflow-hidden">
        <div className={`${(inViewProd === true) && "show-prod-left"} hidden-prod-left lg:col-span-2 flex items-center`}>
            <img src={prod} alt={prod} className="rounded-xl" />
        </div>
        <div className={`${(inViewProd === true) && "show-prod-right"} hidden-prod-right flex flex-col justify-center items-start lg:col-span-3`}>
            <h1 className={"montserrat-font text-3xl text-blue-500"}>Check All Products!</h1>
            <p className={`${(darkMode === true) ? "text-white" : "text-gray-500"} text-start`}>
