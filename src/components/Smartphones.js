  ...
  return (
    <>
        <h1 className="text-center text-4xl font-bold text-blue-500 my-16">New Smartphones</h1>
            <div ref={refPhones} className="grid xl:grid-cols-4 md:grid-cols-2 mx-auto w-2/3 gap-4 pb-10">
                {
                    (smartphones && smartphones.length > 0) &&
                    smartphones.map((phone, ind) => (
                        // style={{transitionDelay:`${ind * 0.25}s`}}
                        <div key={ind} className={`${darkMode ? "hover:bg-gray-500" : "hover:bg-gray-200"} ${(inViewPhones === true) ? "show-phones" : ""} hidden-phones flex flex-col border border-gray-300 rounded-xl h-72 relative cursor-pointer hover:scale-105 duration-300 `}>
